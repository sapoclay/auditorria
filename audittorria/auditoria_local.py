...
from .modelos import ParametrosAuditoria, ResultadoEquipo, ResultadoPuerto, ResumenAuditoria
...
def obtener_certificados_firefox_windows() -> tuple[list[str], list[str]]:
    ...
    if salida.strip():
        ...
        if "enterpriseRoots".lower() in salida_minuscula and "false" in salida_minuscula:
            hallazgos.append("[INFO] Algún perfil de Firefox no parece confiar en raíces empresariales del sistema")
        if "osclientcerts".lower() in salida_minuscula and "true" in salida_minuscula:
...
def auditar_equipo_local(parametros: ParametrosAuditoria) -> ResumenAuditoria:
    """Ejecuta una auditoría local avanzada del equipo actual y empaqueta el resultado."""
    resultado = construir_resultado_local(parametros)
